  ...
  "inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50",
  {
    variants: {
      variant: {
        ...
        expandIcon: "group relative border border-input bg-background hover:bg-accent hover:text-accent-foreground",
...
export interface ButtonProps
  ...
  asChild?: boolean;
  leftIcon?: React.ElementType; // Left icon component
  rightIcon?: React.ElementType; // Right icon component
  onClickLeftIcon?: () => void; // Click handler for left icon
  onClickRightIcon?: () => void; // Click handler for right icon
  showIconOnHover?: boolean; // Control visibility of icons
}
  ...
  (
    ...
    return (
      ...
      >
        {LeftIcon && (
          <div
            ...
            className={`cursor-pointer transition-opacity duration-200 ${
              showIconOnHover ? "opacity-0 group-hover:opacity-100" : "opacity-100"
            }`}
          >
            <LeftIcon />
          </div>
        ...
        {LeftIcon && RightIcon && (
          <div className="border-l border-white mx-2 h-6" />
        )}
